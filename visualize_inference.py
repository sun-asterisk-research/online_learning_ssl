...
models = []
model_names = [
    'teacher', 'student', 'momentum_teacher' # Add more model here to visualize
]

for name in model_names:
    model = OnlineSegmentationModel(
        "FPN", "densenet169", in_channels=3, out_classes=1
    )
    model.model = torch.load("pretrained/fcn_densenet169_{}.pth".format(name))
    models.append(model)
